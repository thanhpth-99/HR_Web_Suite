        ...
        <div class="head-menu col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    <button @click="update()" type="submit" class="btn btn-success w-100 fw-bold me-2"
                    ...
                    <div class="row">
                        <div class="col-8 d-flex justify-content-start">
                            <Button class="btn btn-success me-2" @click="setInfo(ungVien.maUngVien, 2)">Phỏng vấn lần đầu</Button>
                            <Button class="btn btn-success me-2" @click="setInfo(ungVien.maUngVien, 3)">Phỏng vấn lần 2</Button>
                            <Button class="btn btn-success me-2" @click="setInfo(ungVien.maUngVien, 4)">Nhận việc</Button>
                            <button class="btn btn-danger me-2" @click="setInfo(ungVien.maUngVien, 5)">Từ chối</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="card custom-card border-0">
        <div class="row">
            <div class="col-md-9">
                ...
                <input type="text" id="hoTen" :value="ungVien.hoTen" class="form-control">
            </div>
        </div>
    </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" v-model="ungVien.gioiTinh" type="radio" name="gioiTinh" id="nam" :value="true" />
                        <label class="form-check-label" for="nam">Nam</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input v-model="ungVien.gioiTinh" class="form-check-input" type="radio" name="gioiTinh" id="nu" :value="false" />
...
const setInfo = async (maUngVien, trangThai) => {
    ...
    try {
        if (ungVien.value) {
            info.cccd = ungVien.value.cccd;
            info.email = ungVien.value.email;
            info.diaChi = ungVien.value.diaChi;
            info.gioiTinh = ungVien.value.gioiTinh;
            info.hoTen = ungVien.value.hoTen;
            info.dienThoai = ungVien.value.dienThoai;
            info.hinhAnh = ungVien.value.hinhAnh;
            info.ngaySinh = ungVien.value.ngaySinh;
            info.maViTriTuyenDung = ungVien.value.maViTriTuyenDung;
            console.log(info);
            update();
        } else {
            console.error("Không tìm thấy thông tin ứng viên!");
        }
    } catch (error) {
...
const update = async () => {
    try {
        getInfoByMaUngVien(ungVien.value.maUngVien)
